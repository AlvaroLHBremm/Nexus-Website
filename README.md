...
O host Infinityfree não permite com facilidade a requisição de dados diretamente ao website como uma API, ele realiza um "cookie check" pare checar se a requisição vem de um browser ou de uma API automatizada. Para circunver esta limitação foi criado uma vertente do website que requisita os dados localmente, deste modo os "desafios" propostos pelo website seriam executados naturalmente pelo browser do navegador.
